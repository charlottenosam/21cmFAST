...
/*
  Program DRIVE_ZSCROLL_NOTS_SCATTER.C scrolls through the redshifts defined below,
  creating halo, evolved density, velocity, 21cm fields.
  Includes uniform scatter in fesc
...
int main(int argc, char ** argv){
  ...
  // open log file
  system("rm ../Log_files/*");
  LOG = log_open("../Log_files/drive_zscroll_noTs_scatter_log_file");
  if (!LOG){
    fprintf(stderr, "drive_zscroll_noTs_scatter_log_file.c: Unable to open log file\n Aborting...\n");
    return -1;
  }
  ...
  while (Z > (ZEND-0.0001)){
    ...
    // find bubbles
    if (SCATTER){
      // rescale fesc by x2 as average stochastic fesc = 0.5
      fesc_default = ESC_FRAC*2.;
