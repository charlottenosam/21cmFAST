...
int main(int argc, char ** argv){
  ...
  float ion_eff, M, M_MIN, fcoll, x_i, ion_eff_default, fesc_default, fesc;
  char cmnd[1000];
  FILE *LOG;
  ...
  // open log file
  system("rm ../Log_files/*");
  LOG = log_open("../Log_files/drive_xHIscroll_only_log_file");
  if (!LOG){
    fprintf(stderr, "drive_xHIscroll_only_log_file: Unable to open log file\n Aborting...\n");
    return -1;
  }
